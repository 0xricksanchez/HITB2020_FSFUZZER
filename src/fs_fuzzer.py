    ...
    def _exec(self, cmd, to=3):
        ...
        try:
            ...
            if stdout_decoded != "":
                ...
                return None
        except (pm.ssh_exception.SSHException, socket.timeout, pm.ssh_exception.NoValidConnectionsError) as e:
...
def main():
    ...
    parser.add_argument(
        "--user_interaction", "-ui", action="store_true", help="Emulate a user interaction if mount is successful"
    )
    ...
    if args.poc_1:
        fuzzer = Fuzzer(host="192.168.122.232", port=22, fn=["", "/root/poc1_ufs2"], ft="ufs", mntpt="/mnt/HITB/", user_sim=None)
        fuzzer.poc(shell=True)
    if args.poc_2:
        # inject into mnt path
        fuzzer = Fuzzer(host="192.168.122.232", port=22, fn=["", "/root/poc2_ufs2"], ft="ufs", mntpt="/mnt/HITB/", user_sim=None)
        fuzzer.poc(shell=True)
    if args.poc_3:
        # inject into magic bytes
        fuzzer = Fuzzer(
            host="192.168.122.232", port=22, fn=["", "/root/poc3_sb0_ufs2"], ft="ufs", mntpt="/mnt/HITB/", user_sim=None
        )
        fuzzer.poc(shell=True)
    if args.poc_4:
        # radamsa mutated mount crash
        Fuzzer(host="192.168.122.232", port=22, fn=["", "/root/poc4_ufs2"], ft="ufs", mntpt="/mnt/HITB/", user_sim=None).fuzz()
    if args.poc_5:
        # radamsa mutated UI crash with a whole lotta weirdness
        Fuzzer(host="192.168.122.232", port=22, fn=["", "/root/poc5_ufs"], ft="ufs", mntpt="/mnt/HITB/", user_sim=None).poc(
            emul=True
        )
    ...
    if all([args.host, args.port, args.file, args.file_type, args.remote_mount_point]):
        Fuzzer(
            ...
            fn=args.file,
            ft=args.file_type,
            mntpt=args.remote_mount_point,
            user_sim=args.user_interaction,
        ).fuzz()
