...
SUPPORTED_FILE_SYSTEMS = {'freebsd': ['ufs1', 'ufs2', 'zfs', 'ext2', 'ext3', 'ext4'],
...
class GenericFilesystemCreator:
    ...
    def generic_mount(flag, dev, location):
        try:
            subprocess.call(f'/sbin/mount -t {flag} {dev} {location}'.split(), stdout=subprocess.DEVNULL)
            return 1
    ...
    def _hierarchy_sanity_check(self, f_ctr):
        if self.data and self.logger['files'][f'seed_{f_ctr}']['file_name'] != \
                self.data['files'][f'seed_{f_ctr}']['file_name']:
    ...
    def _get_rndm_path_from_lst(self, dirs: List, ignore_system_dirs=False):
        ...
        if ignore_system_dirs:
            if dirs[rndm_idx] not in [os.path.join(self.mount_pt, 'lost+found'), os.path.join(self.mount_pt, '.snap')]:
    ...
    def _get_new_rndm_file_path(self, dirs: List):
        self.rng.seed(self.seed)
        return os.path.join(self._get_rndm_path_from_lst(dirs),
                            self._get_rndm_fname())
    ...
    def _init_fs_dummy_data(self):
        for i, v in list(enumerate(['FILE', 'SYM_LINK', 'DIR'])):
            ...
            if 'FILE' in v:
                ...
                os.symlink(os.path.join(self.mount_pt, _touch_fn),
                           lnk_path)
            else:
                pathlib.Path(_path).mkdir(parents=True, exist_ok=True)
    ...
    def _set_logger_dummy_data(self, name: str, _path: str, i: int, ftype: str):
        ...
        if ftype is 'SYM_LINK':
            self.logger['files']['init_files'][f'init_{i}']['source'] = \
                self.logger['files']['init_files']['init_0']['full_path']

    def _parse_opts(self):
        log_data = None
        parser = argparse.ArgumentParser()
        parser.add_argument('-fs', '--filesystem', type=str,
                            help='ext2, ext3, ext4, ufs1, ufs2, zfs, apfs')
        parser.add_argument('-s', '--size', type=int, default=10,
                            help='Specify the size in MB of the newly created file system, (default: %(default)s)')
        parser.add_argument('-n', '--name', type=str,
                            help='custom name you want to give the file system')
        parser.add_argument('-o', '--output_dir', type=str, default='/tmp/',
                            help='Path to store the newly created file system, (default: %(default)s)')
        parser.add_argument('-p', '--populate', type=int,
                            help='Number of files/directories that will be created on the fresh file system')
        parser.add_argument('-ps', '--populate_size', type=int,
                            help='Max file size limit in KB for -p option')
        parser.add_argument('-mnt', '--mount', type=str, default='/mnt/',
                            help='path to mount the filesystem for populating it, (default %(default)s)')
        parser.add_argument('-m', '--mode', type=int, default=1,
                            help='1 for determinism, or 0 for random (does not use seeds and does no logging), '
                                 '(default: %(default)s)')
        parser.add_argument('-shp', '--shaper', action='append', nargs=2,
        ...
        self.__setup__(fs_name=args.name, fs_type=str(args.filesystem).lower(), fs_size=args.size,
                       n_files=args.populate,
                       max_fsize=args.populate_size, mode=args.mode, save_pt=args.output_dir, data=log_data)
...
class Darwin(GenericFilesystemCreator):
    ...
    def _attach_disk(self):
        hdiutil_out = subprocess.check_output(
            f'/usr/bin/hdiutil attach -imagekey diskimage-class=CRawDiskImage -nomount {self.path}'.split(),
    ...
    def mount_fs(self):
        _mk_dir(self.mount_pt)
        try:
            subprocess.call(f'/sbin/mount_{self.fs_type} {self.dev} {self.mount_pt}'.split(), stdout=subprocess.DEVNULL)
        except subprocess.CalledProcessError:
...
class Ubuntu(GenericFilesystemCreator):
    ...
    def _mk_ufs(self):
        if not _chk_availability('mkfs.ufs'):
            ...
            logging.error('Please install legacy package from:'
        ...
        print(f'[*] The Ubuntu kernel has by default no write permissions for UFS.\n\tEmpty file system \'{self.fs_name}\' created.')
        sys.exit(0)

    def _mk_ext(self):
        subprocess.call(f'/sbin/mkfs.{self.fs_type} -v {self.path}'.split(), stdout=subprocess.DEVNULL)

    def _mk_zfs(self):
        self.fs_name = 'pool_' + self.fs_name
        GenericFilesystemCreator.mountAt = GenericFilesystemCreator._generic_mk_zfs(self.fs_name,
                                                                                    self.dev)

    def mount_fs(self):
        _mk_dir(self.mount_pt)
        try:
            subprocess.call(f'/bin/mount -t {self.fs_type} {self.dev} {self.mount_pt}'.split(),
...
class FreeBSD(GenericFilesystemCreator):
    ...
    def __init__(self, fs, size, name, location, mount_pt, n_files, max_fsize, mode, save_pt, log_data=None):
    ...
    def _mk_ext(self):
        subprocess.call(f'/usr/local/sbin/mkfs.{self.fs_type} -v {self.path}'.split(), stdout=subprocess.DEVNULL)
...
class OpenBSD:
    ...
    def _mk_blk_dev(self):
        subprocess.check_output(f'/sbin/vnconfig vnd0 {self.path}'.split(), stderr=subprocess.STDOUT,
                                encoding='utf-8').strip()
        self.dev = subprocess.check_output('/sbin/disklabel -A vnd0', stderr=subprocess.STDOUT, encoding='utf-8').split()[1][:-1].strip()
        logging.debug(f'block device {self.dev} created')
        return self.dev

    def _unmk_blk_dev(self):
        subprocess.call(f'/sbin/vnconfig -u {self.dev.split("/")[-1]}'.split(), stdout=subprocess.DEVNULL)
...
class NetBSD:
    ...
    def _unmk_blk_dev(self):
        subprocess.call(f'/usr/sbin/vndconfig -u {self.dev.split("/")[-1]}'.split(), stdout=subprocess.DEVNULL)
